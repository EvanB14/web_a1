...
function createImg(src, alt, width) {
  // Replace this comment with your code...
    ...
    if (arguments.length === 3 && width > 0) result = `<img src="${src.trim()}" alt="${alt.trim()}" width="${width}">`;
    else result = `<img src="${src.trim()}" alt="${alt.trim()}">`;
...
function parseDateString(value) {
    ...
    if (!value || !dateRegex.test(value)) throw new Error ("Error");
...
function formatCoord(lat, lng, includeBrackets) {
    ...
    if (validateCoord(lat, lng)) coords = `${lat + "," + lng}`;
    else throw new Error ("Error");

    return includeBrackets ? `[${coords}]` : coords;
...
function formatCoords(...values) {
    ...
    if (count % 2 != 0) throw new Error ("Error");
    ...
    for (let i = 0; i < count; i++) {
        ...
        if ((i + 1) % 2 != 0) {
            ...
            coords.push(arguments[i]);
            if (validateCoord(coords[0], coords[1])) array.push(formatCoord(coords[0], coords[1], true));
            else throw new Error ("Error");
    ...
    for (let i = 0; i < array.length; i++) {
        ...
        if ((i + 1) != array.length) string += `${array[i]}, `;
        else string += array[i];
...
function mimeFromFilename(filename) {
  // Replace this comment with your code...
    ...
    for (let i = 0; i < filename.length; i++) if (filename[i] === ".") count++;

    let ext = filename.split(".")[count];

    if (ext === "html" || ext === "htm") return "text/html";
    else if (ext === "css") return "text/css";
    else if (ext === "js") return "text/javascript";
    else if (ext === "jpg" || ext === "jpeg") return "image/jpeg";
    else if (ext === "ico" || ext === "cur") return "image/x-icon";
...
function generateLicenseLink(licenseCode) {
    ...
    if (licenseCode) {
        ...
        license = `${licenseCode.split(/-(.*)/s)[1]}`

        switch (license) {

          case "BY": link = "Creative Commons Attribution License"; break;
          case "BY-NC": link = "Creative Commons Attribution-NonCommercial License"; break;
          case "BY-SA": link = "Creative Commons Attribution-ShareAlike License"; break;
          case "BY-ND": link = "Creative Commons Attribution-NoDerivs License"; break;
          case "BY-NC-SA": link = "Creative Commons Attribution-NonCommercial-ShareAlike License"; break;
          case "BY-NC-ND": link = "Creative Commons Attribution-NonCommercial-NoDerivs License"; break;
          default: return html;
...
function all() {
    ...
    for (let i = 0; i < arguments.length; i++) if (pureBool(arguments[i]) === false) count++;
...
function none() {
    ...
    for (let i = 0; i < arguments.length; i++) if (pureBool(arguments[i]) === true) count++;
